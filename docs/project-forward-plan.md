...
Planning review: September 8, 2026. Audited model checkout: `ba8bf1a` (code baseline), `613ff12` (documentation revision baseline before the current repair pass). Verify the live checkout with `git rev-parse HEAD` before reproducing a run.
...
| **Training Pipeline** | Overnight 72-epoch TCN trained; GRU interrupted at epoch 52; R0 audit and staged runner controls verified | Full retraining requires 18 hrs across 267k windows; R1 boundary pilot is untrained | **Phase 2**: Run the bounded R1 heads-only warm start, then measure any sampler or partial-unfreeze change rather than assuming a speedup |
...
|---|---|---|---|---|---|
| **Phase 1: Decoder Calibration** | ML Team | `training/src/decoder.py`<br>`training/calibrate_decoder.py` | `artifacts/r0-baseline/decoder/decoder_calibration.json` (local, ignored) | Count MAE $\le 0.40$; Rep End F1 $\ge 60.0\%$ on validation split; R0 count passes but end F1 `0.0`, so gate is blocked | Defines the measured target for Phase 2; does not unlock product runtime |
| **Phase 2: Fast Runner** | ML Team | `training/src/runner.py`<br>`training/train.py` | `training/configs/experiments/r1_tcn_boundary_finetune.yaml` | Controls/tests pass; two-epoch TCN smoke with test locked; full run only after smoke provenance is valid | Unlocks measured fine-tuning comparisons |
